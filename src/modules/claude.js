...
function normalizeSongs(result) {
  ...
  if (!['yue', 'zh', 'en'].includes(result.request_lang)) result.request_lang = null;
  // Guard radio.js's injection logic against a malformed/missing shape —
  // only a real {title: non-empty string} object survives.
  const esr = result.explicit_song_request;
  result.explicit_song_request = (esr && typeof esr.title === 'string' && esr.title.trim())
    ? { title: esr.title.trim(), artist: typeof esr.artist === 'string' ? esr.artist.trim() : '' }
    : null;
  return result;
}
...
1. 判断听众的意图类型：
   - 明确指定硬性条件（歌手名、语言、年代、"最新/latest/new"、曲风/风格）→ 这是硬约束，优先级高于品味档案。品味档案只是"没有明确要求时"的默认值，不是筛选器——本次听众点名的条件哪怕和档案里平时的偏好完全相反，这次也要严格服从听众明确说出来的要求（例：档案显示平时偏好英文歌，但本次点名要粤语歌 → 这次必须全部是粤语歌，档案在这里不再生效）。7 首歌里至少 6 首必须严格满足全部指定条件；最多 1 首可以是相关延伸推荐，但必须在 reason 或 say 里说明为什么加了这一首。如果硬约束是"语言"（比如指定粤语/国语/英文），这条要更严格：7 首必须全部是那个语言演唱的版本，语言类硬约束没有"延伸推荐"这个例外——一首不同语言的歌不能算作对语言要求的合理延伸。当听众本次消息明确要求了某种演唱语言时，把这个语言写进顶层字段 request_lang（'yue'/'zh'/'en'）——服务端会用这个字段做二次过滤，所以它必须和你实际选的 7 首歌的 lang 字段一致，不能写了 request_lang 但选出来的歌里混了别的语言
   - 只描述心情、场景、情绪，没有指定歌手/语言/年代等条件 → 自由发挥，按品味档案和当下氛围选，request_lang 留 null
